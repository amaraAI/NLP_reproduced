# https://github.com/Aleczhang13/MAML-bert/blob/master/functional_forward_bert.py
...
def functional_bert_for_classification(fast_weights, config, input_ids=None, attention_mask=None, token_type_ids=None,
                                       position_ids=None, head_mask=None, inputs_embeds=None,
                                       encoder_hidden_states=None,
                                       encoder_attention_mask=None, labels=None, is_train=True):
    outputs = functional_bert(fast_weights, config, input_ids=input_ids, attention_mask=attention_mask,
    ...
    pooled_output = F.dropout(pooled_output, p=config.hidden_dropout_prob, training=is_train)
    logits = F.linear(pooled_output,
...
def functional_pooler(fast_weights, sequence_output):
    ...
    pooled_output = F.linear(first_token_tensor,
                             fast_weights['bert.pooler.dense.weight'],
                             fast_weights['bert.pooler.dense.bias'])
